...
#>
    ...
    if($ConsentPrompt -Eq 2 -And $SecureDesktopPrompt -Eq 1){
        ...
        $initialExplorerProcesses = Get-Process explorer
        ...
        $finalExplorerProcesses = get-process explorer
        foreach ($fep in $finalExplorerProcesses){
            $found = $false
            foreach ($iep in $initialExplorerProcesses){
                if ($fep.Id -eq $iep.Id) {
                    $found = $true
                    break
                }
